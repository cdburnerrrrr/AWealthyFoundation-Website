...
import { 
  ChevronLeft, ChevronRight, Shield, CheckCircle, Calculator, 
  X, Info, Menu, User, Home, ArrowRight, Target, TrendingUp,
  PiggyBank, CreditCard, DollarSign, Eye, Heart, Star
} from 'lucide-react';
import { 
  FREE_ASSESSMENT_QUESTIONS, 
  generateReport, 
  type AssessmentType,
...
export default function SnapshotQuestionnaire() {
  ...
  const { isAuthenticated, user, saveAssessment } = useAppStore();
  ...
  const questions = FREE_ASSESSMENT_QUESTIONS;
  const totalSteps = questions.length;
  const currentQuestion = questions[currentStep];
  const progress = ((currentStep + 1) / totalSteps) * 100;
  
  // Calculate live scores
  const buildingBlockScores = calculateBuildingBlockScores(responses);
  ...
  const canProceed = () => {
    ...
    if (q.required && !responses[q.key]) return false;
    return true;
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
        ...
        <div className="max-w-4xl mx-auto px-4">
          <div className="flex items-center justify-between h-14">
            ...
            <div className="flex items-center gap-2">
              {isAuthenticated ? (
                <button onClick={() => navigate('/my-foundation')} className="text-sm text-copper-600 font-medium">
                  Dashboard
                </button>
              ) : (
                <button onClick={() => navigate('/login')} className="text-sm text-navy-600 font-medium">
                  Login
                </button>
              )}
            </div>
              ...
              {currentQuestion.type === 'single' && currentQuestion.options && (
                <div className="space-y-3">
                  {currentQuestion.options.map((option) => (
                    <button
                      key={option.value}
                      onClick={() => handleResponse(currentQuestion.key, option.value)}
                  ...
                  {currentQuestion.options.map((option) => {
                    ...
                    return (
                      <button
                        key={option.value}
                        onClick={() => {
                          const current = responses[currentQuestion.key] as string[] || [];
                          const updated = selected
                            ? current.filter(v => v !== option.value)
                            : [...current, option.value];
                          handleResponse(currentQuestion.key, updated);
                        }}
              ...
              <div className="flex items-center justify-between mt-8 pt-6 border-t border-gray-100">
                ...
                {currentStep === totalSteps - 1 ? (
                  <button
                    ...
                    className="flex items-center gap-2 px-6 py-3 bg-copper-600 text-white rounded-lg font-bold hover:bg-copper-700 transition-colors"
                  >
      ...
      {/* Live Score Preview */}
      {Object.keys(responses).length > 3 && foundationScore > 0 && (
        <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            <div className="text-sm text-gray-600">
              Current Score: <span className="font-bold text-navy-900">{foundationScore}</span>
              <span className={`ml-2 text-xs px-2 py-0.5 rounded ${getScoreBand(foundationScore).bg} ${getScoreBand(foundationScore).color}`}>
                {getScoreBand(foundationScore).label}
              </span>
            </div>
            <div className="text-sm text-gray-500">
              {LIFE_STAGE_LABELS[determineLifeStage({ savingsLevel: 3, investingActivity: true })]}
            </div>
          </div>
        </div>
      )}
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <header className="bg-navy-900 text-white py-8">
        <div className="max-w-3xl mx-auto px-4 text-center">
          ...
          <p className="text-navy-300">A personalized financial assessment</p>
        </div>
      </header>
      ...
      <main className="max-w-3xl mx-auto px-4 py-8">
            ...
            {Object.entries(reportData.pillarScores).map(([pillar, score]: [string, number]) => (
              <div key={pillar}>
                <div className="flex justify-between text-sm mb-1">
                  <span className="font-medium text-navy-700">{PILLAR_LABELS[pillar as PillarKey]}</span>
        ...
        <div className="bg-gradient-to-br from-navy-900 to-navy-800 rounded-2xl p-6 text-white text-center">
          ...
          <p className="text-navy-200 mb-4">
            Get a detailed 40+ question assessment with personalized action plan.
          </p>
          <button
            onClick={onUpgrade}
        ...
        <div className="flex justify-center gap-4 mt-6">
          <button
            onClick={onRetake}
            className="text-navy-700 font-medium hover:text-copper-600"
          >
            Retake Assessment
          </button>
          {isAuthenticated && (
